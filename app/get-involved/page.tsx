    ...
    <>
      <section className="pt-32 pb-16">
        <div className="max-w-7xl mx-auto px-6">
          <p className="text-primary text-sm uppercase tracking-[0.3em] mb-6">Get Involved</p>
      ...
      <section className="pb-24">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-3 gap-8">
          {/* Financial */}
          <div className="bg-white border border-border-light hover:border-primary/30 rounded-lg p-8 md:p-10 transition-all duration-300">
            <p className="text-primary text-sm uppercase tracking-[0.3em] mb-4">Financial Contribution</p>
            <h2 className="text-2xl font-heading text-text mb-4">Fund a Future</h2>
            ...
            <div className="space-y-3 mb-8">
              {[
                ...
                <div key={d.amount} className="flex gap-4 items-start">
                  ...
                  <span className="text-text-light text-sm">{d.impact}</span>
                </div>
              ))}
            </div>
            <Link href="/contact" className="inline-block px-6 py-3 bg-accent text-white font-semibold text-xs uppercase tracking-wider rounded-md hover:bg-accent-light transition-colors">
              Contribute
            </Link>
          </div>
          ...
          {/* Goods/Services */}
          <div className="bg-white border border-border-light hover:border-primary/30 rounded-lg p-8 md:p-10 transition-all duration-300">
            <p className="text-primary text-sm uppercase tracking-[0.3em] mb-4">Goods &amp; Services</p>
            <h2 className="text-2xl font-heading text-text mb-4">Donate Resources</h2>
            <p className="text-text-light text-sm leading-relaxed mb-6">
            ...
            <div className="space-y-2 mb-8">
              {["Construction materials & tools", "Office supplies", "Professional services (legal, accounting)", "Training equipment", "Clothing for job interviews"].map((item, i) => (
                <div key={i} className="flex items-start gap-2">
                  <span className="text-primary mt-1 shrink-0"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg></span>
                  <span className="text-text-light text-sm">{item}</span>
                </div>
              ))}
            </div>
            <Link href="/contact" className="inline-block px-6 py-3 border border-primary text-primary text-xs uppercase tracking-wider rounded-md hover:bg-primary/10 transition-colors">
              Donate Goods
            </Link>
          </div>

          {/* Volunteer */}
          <div id="volunteer" className="bg-white border border-border-light hover:border-primary/30 rounded-lg p-8 md:p-10 transition-all duration-300">
            <p className="text-primary text-sm uppercase tracking-[0.3em] mb-4">Volunteer</p>
            <h2 className="text-2xl font-heading text-text mb-4">Give Your Time</h2>
            <p className="text-text-light text-sm leading-relaxed mb-6">
      ...
      <section className="bg-surface py-24">
        <div className="max-w-5xl mx-auto px-6 text-center">
          <p className="text-primary text-sm uppercase tracking-[0.3em] mb-6">Your Impact</p>
          <h2 className="text-3xl md:text-4xl font-heading text-text mb-8">
            Every Dollar <span className="text-primary">Builds Futures</span>
          </h2>
